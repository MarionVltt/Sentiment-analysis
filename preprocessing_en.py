...
def unwrap_SingleRaw_self_get_postag_raw(*arg, **kwarg):
    ...
    for SingleRaw.standardize_raw class method

    """
    return SingleRaw.standardize_raw(*arg, **kwarg)
...
class SingleRaw():
    ...
    STOPWORDS_EN = ["hello", "hi", "hey", "good morning", "bye", "see you"]
    ...
    def standardize_raw(self, sequence_to_remove=[]):
        ...
        no_stop = alpha_only
        for s in SingleRaw.STOPWORDS_EN:
            if s in alpha_only:
    ...
    #
    #     TODO:
    #     enlever stopwords comme Hello, thanks etc avant de tester la longueur
    #
    #     """
    #
...
class RangeRaw():
    ...
    def standardize_range(self, sequence_to_remove=[]):
        ...
        if RangeRaw.multiprocessing:
            self.df_class = Pandas_mpx.map(unwrap_SingleRaw_self_get_postag_raw, self.df_class)
        else:
